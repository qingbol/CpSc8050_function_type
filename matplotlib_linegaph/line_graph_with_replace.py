...
plt.title("Feature Deduction Test with autho's method")
plt.xticks(fea_num)
...
plt.savefig('positive_with.png')
# plt.show()
# -------------------------------------------------------------------------
acc_lemna_neg = np.array(
    [0.260, 0.373, 0.532, 0.665, 0.804, 0.842, 0.911, 0.886, 1])
acc_rand_neg = np.array(
    [0.260, 0.260, 0.342, 0.367, 0.329, 0.443, 0.475, 0.525, 0.519])
...
plt.title("Feature Augmentation Test with autho's method")
plt.xticks(fea_num)
plt.xlabel("feature number")
plt.ylabel("PCR")
plt.gca().yaxis.set_major_formatter(PercentFormatter(1))

plt.savefig('negtive_with.png')
# plt.show()
# -----------------------------------------------------------------------------
acc_lemna_new = np.array(
    [0.076, 0.411, 0.582, 0.728, 0.854, 0.905, 0.911, 0.918, 1])
acc_rand_new = np.array(
    [0.076, 0.310, 0.317, 0.392, 0.411, 0.513, 0.532, 0.576, 0.576])
...
plt.title("Synthetic Test with autho's method")
plt.xticks(fea_num)
plt.xlabel("feature number")
plt.ylabel("PCR")
plt.gca().yaxis.set_major_formatter(PercentFormatter(1))
plt.savefig('new_with.png')
